...
public class App {
    ...
    public static void main(String[] args) {
        ...
        // MongoCollection<Document> people_collection = database.getCollection(info.people_collection);
        // MongoCollection<Document> roi_collection = database.getCollection(info.roi_collection);
        ...
        try{
            do{ //consumer로 받아온 message를 Records 객체에 저장, Records 객체는 Iterable 하다
                ...
                for(ConsumerRecord<String, String> record : records) {
                    ...
                    // Map<String,Object> analyticsModule = (Map<String,Object>) jsonMap.get("analyticsModule");
                    // String CCTV_id = (String) analyticsModule.get("id");
                    // int CCTV_occupancy = (int) analyticsModule.get("occupancy");
                    // int CCTV_roi = (int) analyticsModule.get("roi-OC_count");

                    Document doc = Document.parse(message);
                    // roi cctv message 인 경우
                    // if(CCTV_id.equals("DONGA_A6000_DS6.2_BACK_VIEW")) {
                    //     if(roi_OC_count != CCTV_roi) {
                    //         roi_collection.insertOne(doc);
                    //         roi_OC_count = CCTV_roi;
                    //     }
                    // }
                    // else {
                    //     if(occupancy != CCTV_occupancy) {
                    //         people_collection.insertOne(doc);
                    //         occupancy = CCTV_occupancy;
                    //     }
                    // }
